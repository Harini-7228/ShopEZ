...
const seedDB = async () => {
  try {
    ...
    const catToys        = await Category.create({ name: 'Toys' });
    const catGroceries   = await Category.create({ name: 'Groceries' });
    const catBooks       = await Category.create({ name: 'Books' });
    const catFurniture   = await Category.create({ name: 'Furniture' });
    const catSports      = await Category.create({ name: 'Sports' });
    const catBeauty      = await Category.create({ name: 'Beauty' });
    const catMobiles     = await Category.create({ name: 'Mobiles' });
    const catKitchen     = await Category.create({ name: 'Kitchen' });
    const catHealth      = await Category.create({ name: 'Health & Wellness' });
    const catGaming      = await Category.create({ name: 'Gaming' });
    const catPets        = await Category.create({ name: 'Pet Supplies' });
    const catTravel      = await Category.create({ name: 'Travel' });
    const catBaby        = await Category.create({ name: 'Baby & Kids' });
    ...
    const products = [
      ...
      },
      {
        ...
        images: ['https://images.unsplash.com/photo-1586816879360-004f5b0c51e3?w=600&auto=format&fit=crop&q=80'],
      ...
      },
      {
        ...
        images: ['https://images.unsplash.com/photo-1552667466-07770ae110d0?w=600&auto=format&fit=crop&q=80'],
      ...
      },
      {
        ...
        images: ['https://images.unsplash.com/photo-1586495777744-4e6232bf2ebb?w=600&auto=format&fit=crop&q=80'],
      ...
      // ── KITCHEN ──────────────────────────────────────────────────────
      {
        ...
        images: ['https://images.unsplash.com/photo-1584990347449-39241e96d02b?w=600&auto=format&fit=crop&q=80'],
      ...
      },
      {
        ...
        images: ['https://images.unsplash.com/photo-1647069428929-d70edc17f9a0?w=600&auto=format&fit=crop&q=80'],
      ...
      {
        name: 'TENS/EMS Electric Pulse Massager',
        description: '20 intensity levels for muscle relief and blood circulation improvement. 8 electrode pads, USB rechargeable.',
        price: 1699, discountPrice: 1199,
        category: catHealth._id, sellerId: seller._id, stock: 28, sku: 'HLTH-MASS-03', status: 'active',
        images: ['https://images.unsplash.com/photo-1611071536333-df623d5b5e82?w=600&auto=format&fit=crop&q=80'],
        specifications: { 'Modes': 'TENS + EMS', 'Intensity': '20 Levels', 'Pads': '8 Electrode', 'Power': 'USB Rechargeable' }
      },
