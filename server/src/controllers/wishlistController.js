...
const moveToCart = asyncHandler(async (req, res, next) => {
  ...
  // 4. Remove from wishlist (in-memory splice before saving)
  wishlist.items.splice(itemIndex, 1);

  // Save both documents in parallel. If either save fails, the transaction is rejected.
  try {
    await Promise.all([cart.save(), wishlist.save()]);
  } catch (saveErr) {
    console.error('[wishlist/move-to-cart] Save failure:', saveErr.message);
  ...
  await Promise.all([
    cart.populate({ path: 'items.productId', select: 'name price discountPrice images stock status' }),
    wishlist.populate({ path: 'items.productId', select: 'name price discountPrice images stock status' }),
  ]);
...
export { getWishlist, addRemoveItem, moveToCart, };
