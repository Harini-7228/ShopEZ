      ...
      <Container fluid className="px-3 px-xl-5">
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          ...
          <Nav className="align-items-center ms-auto">
            ...
            {/* Cart Link with Badge — only shown to logged-in customers (Fix #16) */}
            {user && user.role === 'customer' && (
              <Nav.Link as={Link} to="/cart" className="position-relative px-3 me-lg-2">
                <span className="fs-5">🛒</span> <span className="d-none d-lg-inline fw-semibold ms-1">Cart</span>
                {cartItemCount > 0 && (
                  <Badge pill bg="danger" className="position-absolute start-100 translate-middle-x" style={{ fontSize: '0.65rem', top: '2px' }}>
                    {cartItemCount}
                  </Badge>
                )}
              </Nav.Link>
